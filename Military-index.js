...
Weaponry_button.addEventListener('click', () => {
  const contentSections = document.querySelectorAll('.content-section');
  const contentSectionShown = document.querySelector('#content-section-weaponry');
...
Aviation_button.addEventListener('click', () => {
  const contentSections = document.querySelectorAll('.content-section');
  const contentSectionShown = document.querySelector('#content-section-aviation');
  for(const section of contentSections){
    section.classList.add('hidden')
  }
  contentSectionShown.classList.remove('hidden')
...
Waste_Management_button.addEventListener('click', () => {
  const contentSections = document.querySelectorAll('.content-section');
  const contentSectionShown = document.querySelector('#content-section-waste');
  for(const section of contentSections){
    section.classList.add('hidden')
  }
  contentSectionShown.classList.remove('hidden')
...
Facilities_button.addEventListener('click', () => {
  const contentSections = document.querySelectorAll('.content-section');
  const contentSectionShown = document.querySelector('#content-section-facility');
  for(const section of contentSections){
    section.classList.add('hidden')
  }
  contentSectionShown.classList.remove('hidden')
...
// const subtype = feature.properties.Classifica; "filter subcategory from 'Classifica'
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres < 20).filter(f => f.properties.Classifica == 'Military_Weaponry');
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 20 && f.properties.Acres < 500).filter(f => f.properties.Classifica == 'Military_Weaponry');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 500 && f.properties.Acres < 3330).filter(f => f.properties.Classifica == 'Military_Weaponry');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 3330).filter(f => f.properties.Classifica == 'Military_Weaponry');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres < 20).filter(f => f.properties.Classifica == 'Military_Aviation');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 20 && f.properties.Acres < 500).filter(f => f.properties.Classifica == 'Military_Aviation');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 500 && f.properties.Acres < 3330).filter(f => f.properties.Classifica == 'Military_Aviation');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 3330).filter(f => f.properties.Classifica == 'Military_Aviation');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
// Waste Management Brownfield-grid3
// Quartile 1
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres < 20).filter(f => f.properties.Classifica == 'Military_WM');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 20 && f.properties.Acres < 500).filter(f => f.properties.Classifica == 'Military_WM');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 500 && f.properties.Acres < 3330).filter(f => f.properties.Classifica == 'Military_WM');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 3330).filter(f => f.properties.Classifica == 'Military_WM');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres < 20).filter(f => f.properties.Classifica == 'Military_Facilities');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 20 && f.properties.Acres < 500).filter(f => f.properties.Classifica == 'Military_Facilities');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 500 && f.properties.Acres < 3330).filter(f => f.properties.Classifica == 'Military_Facilities');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
...
setTimeout(() => {
  ...
  const refreshImageRow = function () {
    imageRow.innerHTML = '';
    for (let i = 0; i < 6; i++) {
      const quartileFilter = jsonData.features.filter(f => f.properties.Acres > 3330).filter(f => f.properties.Classifica == 'Military_Facilities');
      const index = Math.floor(Math.random() * quartileFilter.length);
      const [lng, lat] = quartileFilter[index].geometry.coordinates;
